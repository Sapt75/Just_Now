...
export default function LabTabs({ status, values }) {
  ...
  async function getFeatures() {
    ...
    if (status) {
      ...
      await Promise.all(Object.keys(res.Features).map((itm) => {
        Object.keys(res.Features[itm]).map((item) => {
          dat[item] = res.Features[itm][item]
        })
      }))


      if (sessionStorage.getItem("features") == null) {
        sessionStorage.setItem("features", JSON.stringify(dat))
      }
      ...
      if (sessionStorage.getItem("features") == null) {
        sessionStorage.setItem("features", JSON.stringify(ses))
      }
  ...
  async function handelInput(e) {
    ...
    let data = JSON.parse(sessionStorage.getItem("features")) === null ? {} : JSON.parse(sessionStorage.getItem("features"))
        ...
        <Box sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <div id='lists'>
            <TabList onChange={handleChange} aria-label="lab API tabs example">
              {data ? Object.keys(data.Features).map((item, index) => {
        ...
        {data ? Object.keys(data.Features).map((item, index) => {
          return (<TabPanel value={`${index + 1}`}>
            <div className='w-100 border-2 border-black'>
              {Object.keys(data.Features[item]).map((itm, index) => {
                return (<div className={`mx-[4rem] ${index === Object.keys(data.Features[item]).length - 1 ? "mb-[3rem]" : null}`}>
